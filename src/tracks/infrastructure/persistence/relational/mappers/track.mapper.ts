...
export class TrackMapper {
  ...
  toEntity(domain: Track | Partial<Track>): TrackEntity {
    ...
    if (domain.transcodedObjectKey !== undefined) entity.transcodedObjectKey = domain.transcodedObjectKey;
    if (domain.contentType) entity.contentType = domain.contentType;
    if (domain.size !== undefined) entity.size = domain.size;
    if (domain.durationSeconds !== undefined) entity.durationSeconds = domain.durationSeconds;
    ...
    if (domain.mainArtists !== undefined) entity.mainArtists = domain.mainArtists;
    ...
    if (domain.scheduledAt !== undefined) entity.scheduledAt = domain.scheduledAt;
    
    // Advanced details
    if (domain.buyLink !== undefined) entity.buyLink = domain.buyLink;
    if (domain.recordLabel !== undefined) entity.recordLabel = domain.recordLabel;
    if (domain.releaseDate !== undefined) entity.releaseDate = domain.releaseDate;
    if (domain.publisher !== undefined) entity.publisher = domain.publisher;
    if (domain.isrc !== undefined) entity.isrc = domain.isrc;
    if (domain.containsExplicitContent !== undefined) entity.containsExplicitContent = domain.containsExplicitContent;
    if (domain.pLine !== undefined) entity.pLine = domain.pLine;
    
    // Permissions
    if (domain.enableDirectDownloads !== undefined) entity.enableDirectDownloads = domain.enableDirectDownloads;
    if (domain.enableOfflineListening !== undefined) entity.enableOfflineListening = domain.enableOfflineListening;
    if (domain.includeInRssFeed !== undefined) entity.includeInRssFeed = domain.includeInRssFeed;
    if (domain.displayEmbedCode !== undefined) entity.displayEmbedCode = domain.displayEmbedCode;
    if (domain.enableAppPlayback !== undefined) entity.enableAppPlayback = domain.enableAppPlayback;
    
    // Engagement privacy
    if (domain.allowComments !== undefined) entity.allowComments = domain.allowComments;
    if (domain.showCommentsToPublic !== undefined) entity.showCommentsToPublic = domain.showCommentsToPublic;
    if (domain.showInsightsToPublic !== undefined) entity.showInsightsToPublic = domain.showInsightsToPublic;
    if (domain.geoblockingType !== undefined) entity.geoblockingType = domain.geoblockingType;
    if (domain.allowedRegions !== undefined) entity.allowedRegions = domain.allowedRegions;
    if (domain.blockedRegions !== undefined) entity.blockedRegions = domain.blockedRegions;
    
    // Audio preview
    if (domain.previewStartTime !== undefined) entity.previewStartTime = domain.previewStartTime;
    
    // Licensing
    if (domain.licenseType !== undefined) entity.licenseType = domain.licenseType;
